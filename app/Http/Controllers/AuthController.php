...
//            try {
            ($request->has('reset')) ?
                ...
                $request->validate(['number' => 'required|not_empty|unique:users|phone_number', 'major' => 'required_major', 'invitation_code' =>
...
//            return response()->json(['createPasswordEmail' => $request->all()],200);
    ...
    public function SmsVerificationCode(Request $request)
    {
        ...
        $validated = Validator::make($request->all(), [
            ...
            'code' => ($request->has('validatecode')) ?
                ['required', 'integer', Str::contains(Route::currentRouteName(), 'api') ? '' : new CheckEmailVerificationCode()] : '',
    ...
    public function VoiceVerificationCode(Request $request)
    {
        ...
        if ($request->has('verifyBytel')) {
            ...
            if (isset($request->reset)) {
                ...
                if (Str::contains(Route::currentRouteName(), 'api')) {
                    return response()->json(['next_url' => \route('api.auth.SmsVerification-code'), 'expected_inputs' => 'number,major,code,invitation_code,password,password_confirmation', 'old_values' => $request->input()], 200);
                }
                return view('auth.createPasswordSms', $request->input());
            }
        }
